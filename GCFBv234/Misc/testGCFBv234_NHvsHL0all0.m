...
SwEqlzMds = 1; % convetional　　v231で使っていたのはこちら。
SwEqlzMds = 2; % new feature 　 v233以降、これでも補正ができるように。
...
for SwDySt =  1%:2 % 1 % only dynamic
    ...
    for SwSPL = 1:length(SndSPLdBList)
        ...
        % HL0で、all 0にしたときにNHと同じになるかをチェック 8 Sep 2022
        GCparam.HLoss.Type = 'HL0';
        GCparam.HLoss.HearingLeveldB = [ 0 0 0 0 0 0 0];   % equivalent to NH 
        %GCparam.HLoss.HearingLeveldB = [ 0 0 0 0 0 0 0]+0.01;  
        GCparam.HLoss.CompressionHealth =  1; % equivalent to NH 
        % GCparam.HLoss.CompressionHealth =  0.5; % HL0で、all 0の時は、1に補正されることをチェックするために入れた。確認済み。
        ...
        DiffEP =  sqrt(mean(mean((dcGCframe-dcGCframeHL).^2)))/sqrt(mean(mean((dcGCframe).^2)))
        DiffEPdB =  20*log10(DiffEP)
