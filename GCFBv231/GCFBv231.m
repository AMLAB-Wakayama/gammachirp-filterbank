...
[GCparam, GCresp] = GCFBv231_SetParam(GCparam);  %
fs          = GCparam.fs;
NumCh = GCparam.NumCh;
Tstart   = clock;
...
if strncmp(GCparam.Ctrl,'dyn',3) == 1  % Dynamic
    if strncmp(GCparam.DynHPAF.StrPrc,'sample',5) == 1
        if  strncmp(GCparam.HLoss.Type,'NH',2) ~= 1
            warning(['The output of GCFBv231_SampleBase has not been checked for ' GCparam.HLoss.Type]);
        end
        [dcGCsmpl,  GCresp] = GCFBv231_SampleBase(pGCsmpl, scGCsmpl, GCparam, GCresp);
        cGCout = dcGCsmpl;   % Sample output
        %
     elseif strncmp(GCparam.DynHPAF.StrPrc,'frame',5) == 1
        [dcGCframe, GCresp] = GCFBv231_FrameBase(pGCsmpl, scGCsmpl, GCparam, GCresp);
...
% adhoc compensation:  it is not very good  3 Sep 21
% NHはこれで補正できるが、HLが異なる場合は、かなりずれることになる。
% これはIOfunctionの動作点（入力側）の位置の違いによる出力（のスロープ）の違いによるのかも。
%
% NG FB_FineTunedB = GainFactor_FineTune(GCparam);
% GainFactor= 10.^(-(GCparam.HLoss.FB_AFgainCmpnstdB+FB_FineTunedB)/20);
% GainFactor= 10.^(-(GCparam.HLoss.FB_AFgainCmpnstdB+GCparam.HLoss.FB_AFgainCmpnstdB_FineTune)/20);   % HL0dB, HL val dBを一致させるため
% function [FB_FineTunedB] =  GainFactor_FineTune(GCparam)
%     FineTunedB = [3, 0, -3, -3, -3, -3, -3];
%     ERBrateFag = Freq2ERB(GCparam.HLoss.FaudgramList);
%     ERBrateFr1 = Freq2ERB(GCparam.Fr1);
%     FB_FineTunedB  = interp1(ERBrateFag,FineTunedB, ERBrateFr1,'linear','extrap');
% end

   % 以下の補正をすると、従来法とのcompativilityが崩れる。WHISでも計算がややこしくなる。
   %  RMSlevel_AbsThreshold = 10^(-30/20);             % rms valueこの値がOutput == 0dB ---   GCFBでのMedisHairCellの0dB分だけ補正。
                                                                             %  GCFBでのMedisHairCellの0dB    rms(s(t)) == sqrt(mean(s.^2)) == 1   --> 30 dB SPL    
   %  dcGCout = RMSlevel_AbsThreshold*cGCout;      % output name dcGCout　rms値を取れば、そのままrms出力値になるように。
   % ---> 必要なら、GCparam.MeddisIHCLevel_RMS0dB_SPLdBを用いること

    
    % 閾値下を一定とする場合。現状では適用せず。現状のGEDI等とのcompatibilityのためには、入れない方が良い。
    % 必要ならnoise floorを入れるべきか。　10 Feb 2021
    %     if strncmp(GCparam.DynHPAF.StrPrc, 'frame',5) == 1,
    %         %   cGCout = max(cGCout,RMSlevel_AbsThreshold);  % これ以下の出力は閾値下
    %     end;
    
% HLossdB_OHC -- Cochlear Health分
% c2val_CH = GCresp.c2val; c2val_CH  = 3;   c2val_CH  = 0;  c2val_CH  = GCresp.c2val*2;     結果同じで固定なのでダメ。
% fratRef = GCresp.frat0val + GCresp.frat1val.*GCparam.HLoss.InputdB_For_Normalize;
% fratRef1 = GCresp.frat0Pc +GCresp.frat1val.*(GCparam.HLoss.NormInputdB - GCresp.PcHPAF); と同じ
% c2val_CH = GCresp.c2val.*GCparam.HLoss.FB_CompressionHealth; % 　24 Jul 20
% cGCRef = CmprsGCFrsp(GCresp.Fr1,fs,GCparam.n,GCresp.b1val,GCresp.c1val,fratRef,GCresp.b2val,c2val_CH);
% GCresp.GainFactor_OHC = cGCRef.NormFctFp2;
%  GCresp.GainFactor_OHC = 10.^(-GCparam.HLoss.FB_HLossdB_OHC/20);   % OHC_GainReductは削除すみ    



    
    % HLossdB_IHC 分 GainReduct --> See "CalGCHearingLoss.m" in "GCFBv230_SetParam.m"
    % magic number--- 消去すること 28 Aug 21
    % NHのAbsThrがHL0に近くなるように調整 magic numberでいまいちだが、しかたない。
    % GCresp.GainFactor_NormIOfunc_CmpnstTermdB = 50; % NHの時、HL0との差分が5dB以下となる。good. 決め打ち。　22 Jan 2021   AmpNormAFG_pi2の場合
    % GCresp.GainFactor_NormIOfunc_CmpnstTermdB = 38; % 17 Aug 2021  -- GCFBv230で、AmpNormAFG_IOEqを使う場合 NG

    % GCresp.GainFactor_NormIOfunc = 10.^( (GCparam.HLoss.FB_OutputdB_For_Normalize - GCresp.GainFactor_NormIOfunc_CmpnstTermdB) /20);
    % GCresp.GainFactor_NormIOfunc = 1; これより上の方が良い。
    %GCparam.HLoss.FB_OutputdB_For_Normalizeとは？

    
     %% DEBUG_MODE %%%
     %GCparam.HLoss
     %GCsmpl1 = pGCsmpl(nch,:); % ACFすら抜かす
     % scGCresp.NormFctFp2(nch) = 1;  % チェック
     %%%
     % scGCsmpl(nch,:) = GCsmpl1; % static compressive GC output : sample-by-sample
     % 
     
     
             % pGCframe = GCresp.pGCframe; 
        % scGCframe = GCresp.scGCframel
        % LvldBframe = GCresp.LvldBframe
        
            % HearingLossにも対応する。　GCFBv231_HearingLossで値は算出済み
    % Output level is normalized to make the absolute threshold be 0 (dB).
    % Using GCparam.HLoss.InputdB_For_Normalize in CalGCHearingLoss.m
    %GCresp.GainFactor_OHC = 1; % dcGCFBの計算の中で、すでに、OHC分は、低減されているので、出力側で補正はしてはいけない。
    % GCresp.GainFactor_IHC= 10.^(-GCparam.HLoss.FB_IOfuncLossdB_IHC/20); % Loss(positive) --> Gain(negative)
    % AsymFuncInOutの出力レベルでnormalizeするので、これも不要。
    %GCresp.GainFactor_IHC = 1;
    %GainFactor = GCresp.GainFactor_IHC.*GCresp.GainFactor_OHC.*GCparam.GainFactor_HLval;
    % +GCparam.HLoss.FB_AFgainCmpnstdB_FineTuneによる補正は無い方が結果は良い。
    % すっきりさせるため消去