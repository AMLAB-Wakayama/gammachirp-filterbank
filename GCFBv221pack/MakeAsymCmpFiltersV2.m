...
%  Modified:  11 May 2020 (adding output of convoluted coefficients)
%
...
%               bz : MA coefficients  (NumCh*9)  (convolution of 4 filters)
%               ap : AR coefficients  (NumCh*9)
%
function [ACFcoef, ACFcoefConv] = MakeAsymCmpFiltersV2(fs,Frs,b,c)
...
if nargin <= 1
    return;
else %% when ACFcoefConv is necessary
    ACFcoefConv.ap = zeros(NumCh,9);
    ACFcoefConv.bz = zeros(NumCh,9);  
    for nch = 1:NumCh
        ap1 = 1;
        bz1 = 1;
        for Nfilt = 1:4
            ap1 = conv(ACFcoef.ap(nch,:,Nfilt),ap1);
            bz1 = conv(ACFcoef.bz(nch,:,Nfilt),bz1);
        end;
        % length(ap1) == 9
        ACFcoefConv.ap(nch,1:9) = ap1;
        ACFcoefConv.bz(nch,1:9) = bz1;
    end;
end;
