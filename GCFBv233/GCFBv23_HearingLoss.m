...
for nFag = 1:LenFag
    ...
    % IOfuncLossdB_PAS(nFag) = 0; % default
    ...
    if PinLossdB_PAS(nFag) == 0  && GCparam.HLoss.HearingLeveldB(nFag) > 0
        ...
        CompressionHealth = interp1(PinLossdB_ACT4Cmpnst,CmprsHlthList, PinLossdB_ACT(nFag)); % 最も近いものを探す-- 補正した値
...
% GCparam.HLoss.IOfuncLossdB_PAS    = IOfuncLossdB_PAS;
